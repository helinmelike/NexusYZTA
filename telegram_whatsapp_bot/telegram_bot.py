...
from marketplace_service import (
    ...
    sync_hepsiburada_orders,
    get_channel_report,
    get_zero_stock_marketplace_warnings,
    CHANNEL_LABELS,
)
...
BTN_MP_TRENDYOL = _btn(seller_marketplace_menu, 0)
BTN_MP_HEPSIBURADA = _btn(seller_marketplace_menu, 1)
BTN_MP_REPORT = _btn(seller_marketplace_menu, 2)
BTN_MP_STOCK_WARN = _btn(seller_marketplace_menu, 3)
WEB_SUPPORT_MESSAGE = (
...
MENU_BUTTON_TEXTS = {
    ...
    BTN_MP_TRENDYOL, BTN_MP_HEPSIBURADA, BTN_MP_REPORT, BTN_MP_STOCK_WARN,
}
...
def _fmt_marketplace_sync(result: dict) -> str:
    if not result.get("success"):
        return f"❌ Hata: {result.get('message', 'Bilinmeyen hata')}"

    channel = result.get("channel", "")
    label = CHANNEL_LABELS.get(channel, channel.capitalize())
    imported = result.get("orders", [])
    skipped = result.get("skipped_count", 0)

    if not imported:
        return f"✅ {label}: Yeni sipariş bulunamadı. ({skipped} tekrar atlandı)"

    lines = [f"✅ <b>{label} — {len(imported)} yeni sipariş alındı</b>\n"]
    stock_warnings = []

    for o in imported[:10]:
        ch_icon = "🟠" if channel == "trendyol" else "🟡"
        lines.append(
            f"{ch_icon} <code>{o['order_number']}</code>\n"
            f"   👤 {o['customer_name']}\n"
            f"   📦 {o['product_name']} × {o['quantity']}\n"
            f"   💰 {o['total_amount']} TL"
        )
        if o.get("stock_warning"):
            stock_warnings.append(o["product_name"])

    if skipped:
        lines.append(f"\n⏭ {skipped} sipariş zaten sistemde, atlandı.")

    if stock_warnings:
        lines.append(f"\n⚠️ <b>Stok Uyarısı:</b> {', '.join(stock_warnings)} stoğu tükendi — marketplace'te pasife alınmalı!")

    return "\n".join(lines)


def _fmt_channel_report(result: dict) -> str:
    if not result.get("success"):
        return f"❌ Rapor alınamadı: {result.get('message', '')}"

    date_str = result.get("date", "bugün")
    channels = result.get("channels", {})
    total_orders = result.get("total_orders", 0)
    total_revenue = result.get("total_revenue", 0.0)

    lines = [f"📊 <b>Günlük Kanal Raporu — {date_str}</b>\n"]
    ...
    for ch, data in channels.items():
        ...
        lines.append(
            f"{icon} <b>{label}</b>\n"
            f"   📦 {data['order_count']} sipariş\n"
            f"   💰 {data['revenue']:.2f} TL"
        )

    lines.append(f"\n{'─' * 25}")
    lines.append(f"📦 Toplam Sipariş: <b>{total_orders}</b>")
    lines.append(f"💰 Toplam Ciro: <b>{total_revenue:.2f} TL</b>")
...
async def _seller_menu_router(update: Update, text: str, sess: UserSession) -> None:
    if text == BTN_S_ORDERS:
        ...
        await _reply(update, "🛒 <b>Marketplace Yönetimi</b>\n\nTrendyol ve Hepsiburada siparişlerini buradan yönetebilirsiniz.", seller_marketplace_menu())
    elif text == BTN_S_REPORTS:
        # Operasyonel özet
        op_result = _seller_report_summary_local()
        d = op_result["data"]
        # Kanal bazlı rapor
        ch_result = get_channel_report()
        ch_lines = []
        if ch_result.get("success"):
            channel_icons = {"trendyol": "🟠", "hepsiburada": "🟡", "direct": "🟢"}
            for ch, data in ch_result.get("channels", {}).items():
                icon = channel_icons.get(ch, "🔵")
                label = CHANNEL_LABELS.get(ch, ch.capitalize())
                ch_lines.append(f"  {icon} {label}: {data['order_count']} sipariş / {data['revenue']:.0f} TL")
        channel_section = "\n".join(ch_lines) if ch_lines else "  Bugün henüz sipariş yok."
        await _reply(
            update,
            "<b>📊 Günlük Rapor Özeti</b>\n\n"
    ...
    elif text == BTN_MP_TRENDYOL:
        await _reply(update, "🟠 Trendyol siparişleri çekiliyor...", seller_marketplace_menu())
        result = sync_trendyol_orders()
        await _reply(update, _fmt_marketplace_sync(result), seller_marketplace_menu())
    elif text == BTN_MP_HEPSIBURADA:
        await _reply(update, "🟡 Hepsiburada siparişleri çekiliyor...", seller_marketplace_menu())
        result = sync_hepsiburada_orders()
        await _reply(update, _fmt_marketplace_sync(result), seller_marketplace_menu())
    elif text == BTN_MP_REPORT:
        result = get_channel_report()
        await _reply(update, _fmt_channel_report(result), seller_marketplace_menu())
    elif text == BTN_MP_STOCK_WARN:
        warnings = get_zero_stock_marketplace_warnings()
        if warnings:
            lines = ["⚠️ <b>Stoğu Tükenen Ürünler</b>\n", "Aşağıdaki ürünler marketplace'te pasife alınmalıdır:\n"]
            for name in warnings:
                lines.append(f"❌ {name}")
            await _reply(update, "\n".join(lines), seller_marketplace_menu())
        else:
            await _reply(update, "✅ Tüm ürünlerin stoğu yeterli. Marketplace'te pasife alınacak ürün yok.", seller_marketplace_menu())
    else:
        await _reply(
            update,
